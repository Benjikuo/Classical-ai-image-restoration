...
# Five degradation types used in this project.
DEGRADATION_TYPES = ["dark", "low_contrast", "blurry", "noisy", "color_shift"]

# Fixed random seed to make Gaussian noise reproducible.
RANDOM_SEED = 123


def degrade_dark(img):
    """
    Simulate underexposure by reducing image intensity.
    """
...
def degrade_low_contrast(img):
    """
    Simulate low contrast by compressing pixel intensities toward middle gray.
    """
...
def degrade_blurry(img):
    """
    Simulate blur using Gaussian blur.
    """
...
def degrade_noisy(img):
    ...
    The result is reproducible because RANDOM_SEED is set in generate_degraded_dataset().
    """
...
def degrade_color_shift(img):
    """
    Simulate color shift by changing BGR channel strengths.
...
def generate_degraded_dataset(dataset_dir: Path):
    ...
    Required metadata.csv column:
    - filename

    Optional metadata.csv column:
    - prompt
    ...
    for _, row in metadata.iterrows():
        ...
        for degradation in DEGRADATION_TYPES:
            degrade_function = DEGRADE_FUNCTIONS[degradation]
            degraded = degrade_function(img)
    ...
    output_path = dataset_dir / "degraded_metadata.csv"
